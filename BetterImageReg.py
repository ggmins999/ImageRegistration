...
def drawGrid(img,xoff,yoff):
    ...
    before_thresh = doThreshold(before_gray)
    img = cv2.cvtColor(before_thresh, cv2.COLOR_GRAY2BGR)

    cv2.putText(img, f'gridresponse = {findGridResponse(before_thresh, dx, dy, gridx, gridy, r_angle, annotate=img)}', (0,230), font, 1, (200,255,155), 2, cv2.LINE_AA)
...
def countPixelsAtGridPoint (img,radius,point):
    ...
    slice = img[y1:y2, x1:x2]
    return  1 if cv2.countNonZero(slice) > 2 else 0



def findGridResponse(img, xoff, yoff, gx, gy, angle, annotate=None):
    response = 0
    num = 1
    radius = 5
    w = img.shape[0]
    ...
    for n, p1 in enumerate(gridpoints(img, xoff, yoff, gx, gy, max_radius=400)):
        #if n%2 != 0:
        #    continue
        npixels = countPixelsAtGridPoint(img_rotated,radius,p1)
        if annotate is not None and npixels > 0:
            cv2.circle(annotate, p1, radius, (255, 0, 0), 1)  # draws circle for nonzero pixel count
        response = response + npixels
    return response
...
def findMaxGridResponse(img):
    max_response = 0
    max_params = (0,0,0,0,0) # record what the params were when response was maximum
    n = 0
    for angle in np.arange(r_angle-1, r_angle+1, 0.25):
        for xoff in np.arange(dx-2,dx+2,1):
            for yoff in np.arange(dy-2,dy+2,1):
                for gx in np.arange(gridx-2,gridx+2,0.5):
                    for gy in np.arange(gridy-2,gridy+2,0.5):
                        resp = findGridResponse(img,xoff, yoff, gx, gy, angle)
                        n = n + 1
                        print(n,xoff,yoff,gx,gy,resp)
                        if resp > max_response:
                            max_response = resp
                            max_params = (xoff,yoff,gx, gy, angle)
...
while True:
    ...
    if chr(key) == 'r':
        ...
        dx,dy,gridx,gridy,r_angle = findMaxGridResponse(before_thresh)
    elif chr(key) == '1':
        r_angle += 0.25
    elif chr(key) == '2':
        r_angle -= 0.25
